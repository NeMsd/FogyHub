...
📝 Твой основной код (source.lua на GitHub) [1.3.1]:

(Можешь засунуть его в Prometheus и залить обфусцированную версию) [1.3.1]
    ...
    addConnection(RunService.Heartbeat:Connect(function()
        pcall(function()
            ...
            if knife and char and char:FindFirstChild("HumanoidRootPart") then
                if knife.Parent == bp then char.Humanoid:EquipTool(knife) end
                for _, victim in ipairs(Players:GetPlayers()) do
                    if victim ~= LocalPlayer && victim.Character and victim.Character:FindFirstChild("HumanoidRootPart") then
                        local vHum = victim.Character:FindFirstChildOfClass("Humanoid")
