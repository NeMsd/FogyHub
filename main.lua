--[[
    ========================================================================
    FOGYHUB — MM2 MULTI-TOOL (REBUILT & OPTIMIZED)
    ========================================================================
    * Version: 2.1.0
    * Authors: MsD, Gemini, Vertex Logic Integration
    * License: MIT (Feel free to copy, modify, and distribute)
    * Platform: Roblox (Luau)
    * Compatibility: PC & Mobile Executors (Solara, Celery, Delta, Codex, etc.)
    ========================================================================
]]

-- =========================================================================
-- [MODULE 1: ENVIRONMENT & INITIAL COMPATIBILITY SAFETY LAYER]
-- =========================================================================

-- Safe global environment retrieval
local gp = getgenv and getgenv() or _ENV or _G

-- Polyfill for executors missing the native 'newproxy' function (e.g. patched Delta/Solara)
if not gp.newproxy then
    warn("[FogyHub Safety]: native newproxy is missing. Injecting emulator polyfill...")
...
-- Safely clear previous FogyHub threads, render steps, and UI instances to avoid memory leaks
if gp.FogyHub_Cleanup then
    local success, err = pcall(gp.FogyHub_Cleanup)
    if not success then
        warn("[FogyHub Cleanup Error]: " .. tostring(err))
    end
end

-- Initialize persistent execution storage
gp.FogyHub_Connections = {}
gp.FogyHub_Binds = {}
gp.FogyHubActive = true

-- Garbage collector & clean execution controller
gp.FogyHub_Cleanup = function()
    gp.FogyHubActive = false
    
    -- Disconnect all RBXScriptConnections registered in this session
    for index, connection in ipairs(gp.FogyHub_Connections or {}) do
        if connection then
            pcall(function()
    ...
    -- Unbind all priority render steps bound to RunService
    for index, bindName in ipairs(gp.FogyHub_Binds or {}) do
        pcall(function()
    ...
    -- Destroy any residual graphical interfaces or mobile binds screen guis
    pcall(function()
    ...
    print("[FogyHub]: Cleaned up active processes successfully.")
end

-- Local registration helper for tracking script connections
local function addConnection(conn)
    table.insert(gp.FogyHub_Connections, conn)
    return conn
end

-- Local registration helper for tracking RunService render steps
local function addBind(name, priority, func)
    pcall(function()
        game:GetService("RunService"):BindToRenderStep(name, priority, func)
    end)
    table.insert(gp.FogyHub_Binds, name)
end

-- Fallback safety for clipboard operations
local setclipboard = setclipboard or toclipboard or (Clipboard and Clipboard.set) or function(text)
    warn("[FogyHub Clipboard Error]: Clipboard functions are not supported by your executor.")
    print("[Console Output]: " .. tostring(text))
end

-- =========================================================================
-- [MODULE 2: CORE SERVICES & CONFIGURATION MANAGEMENT]
-- =========================================================================
...
-- Centralized configuration table storing state for all features
local Config = {
    Language = "en",
    Visuals = {
        MurdererESP = false,
...
-- Write current configuration state to local filesystem JSON file
local function saveConfig()
    if writefile then
        pcall(function()
            local serialized = HttpService:JSONEncode(Config)
            writefile(configFileName, serialized)
        end)
    end
end

-- Read configuration from filesystem and apply to the settings table
local function loadConfig()
    if readfile and isfile and isfile(configFileName) then
        pcall(function()
            local rawContent = readfile(configFileName)
            local loadedData = HttpService:JSONDecode(rawContent)
            if loadedData then
                -- Parse and safely map loaded fields to prevent structure breaks
                if loadedData.Language then 
                    Config.Language = loadedData.Language 
                end
                if loadedData.Visuals then 
                    for key, val in pairs(loadedData.Visuals) do 
                        Config.Visuals[key] = val 
                    end 
                end
                if loadedData.Combat then 
                    for key, val in pairs(loadedData.Combat) do 
                        Config.Combat[key] = val 
                    end 
                end
                if loadedData.Utility then 
                    for key, val in pairs(loadedData.Utility) do 
                        Config.Utility[key] = val 
                    end 
                end
                if loadedData.MobileButtons then 
                    for key, val in pairs(loadedData.MobileButtons) do 
                        Config.MobileButtons[key] = val 
                    end 
                end
                if loadedData.ButtonPositions then 
                    for key, val in pairs(loadedData.ButtonPositions) do 
                        Config.ButtonPositions[key] = val 
                    end 
                end
...
-- =========================================================================
-- [MODULE 3: BILINGUAL TRANSLATION DICTIONARY]
-- =========================================================================
...
local L = {
    en = {
        ...
        NoH = "Hero not found or dead", 
        SitError = " is sitting (fling impossible)", 
        Flinging = "Flinging: ", 
        RadioNoSupport = "Your executor does not support writefile/getcustomasset!",
        ...
        VisualsSkinChanger = "Visual Skins",
        ...
        BtnShootM = "Button: Shoot", 
        GodMode = "Dodge Knife", 
        BtnGodMode = "Button: Dodge Knife",
        SilentAim = "Silent Aim (Shoot Without Turning Camera)", 
        AutoFarmCoins = "Auto-Farm Coins (Safe Glide)", 
        EmoteSpam = "Emote Glitch Spammer (Zen/Sit)", 
        ChatAlerts = "Local Role Notifications in Chat",
        MMBWarningTitle = "⚠️ Custom Server Detected", 
        MMBWarningContent = "You are playing on an unofficial copy of MM2 (e.g. MMB). Some features like Skin Changer or Role tracking may be unstable."
    },
    ru = {
        CrashTitle = "🚨 FogyHub — Пиздец Приехали (Краш)", 
        CopyLog = "Скопировать этот лог", 
        Copied = "Скопировано, бля!", 
        BufError = "Буфер обосрался!", 
        Close = "Закрыть залупу", 
        Loaded = "FogyHub загружен, нахуй!", 
        FailedUI = "Не удалось стянуть WindUI.",
        Visuals = "Визуалы, сука", 
        Combat = "Бойня, бля", 
        Utility = "Всякая хуйня", 
        MobileBinds = "Тел. Кнопки", 
        Radio = "Радио", 
        Teleports = "Телепорты", 
        Configs = "Конфиги / Настройки",
        EspM = "ESP на пидораса с ножом", 
        EspS = "ESP на лоха с пушкой", 
        EspI = "ESP на мирное мясо", 
        EspBoxes = "3D Коробки (для бомж-читов)", 
        Stretch = "Растяг экрана (4:3)", 
        StretchForce = "Сила растяга",
        NoFog = "Снести туман к хуям", 
        TimeOfDay = "Время суток", 
        EspG = "ESP на пестик под ногами", 
        Invis = "Стать призраком (Невидимка)", 
        BtnInvis = "Кнопка: Невидимка",
        AutoShoot = "Авто-выстрел в ебало мардера", 
        Aimlock = "Аим на башку убийцы", 
        KillAura = "Килаура (нагибать всех)", 
        KillAuraRange = "Радиус бойни", 
        AutoKillAll = "Геноцид всего сервера",
        FlingM = "Выбить мардера в космос", 
        FlingS = "Выбить шерифа нахуй", 
        FlingH = "Выбить героя", 
        AutoGrab = "Пиздить пестик с земли", 
        SlideGlitch = "Спидглитч бега", 
        SlideSpeed = "Скорость бега", 
        Noclip = "Проход сквозь стены", 
        AntiFling = "Анти-Флинг",
        LockButtons = "Запереть кнопки на экране", 
        BtnScale = "Масштаб кнопок", 
        BtnFlingM = "Кнопка: Флинг Мардера", 
        BtnFlingS = "Кнопка: Флинг Шерифа", 
        BtnFlingH = "Кнопка: Флинг Героя", 
        BtnGrab = "Кнопка: Подбор пестика", 
        BtnSlide = "Кнопка: Спидглитч",
        BtnNoclip = "Кнопка: Ноуклип", 
        BtnKillAura = "Кнопка: Килаура", 
        BtnKillAll = "Кнопка: Геноцид", 
        RobloxId = "ID Музла из Roblox", 
        PlayId = "Играть музло по ID", 
        HttpUrl = "Прямая ссылка на MP3/OGG",
        PlayHttp = "Врубить музыку по ссылке", 
        StopRadio = "Заткнуть радио", 
        Volume = "Громкость", 
        Loop = "Зациклить", 
        NoKnife = "Нож не найден, бля!", 
        NoGun = "Пестика у шерифа нет!", 
        NoM = "Убийца подох или не найден",
        NoS = "Шериф подох или не найден", 
        NoH = "Герой подох или не найден", 
        SitError = " сидит, хуй ты его выбьешь!", 
        Flinging = "Ебашим: ", 
        RadioNoSupport = "Твой эксплойт — говно без поддержки getcustomasset!",
        RadioDownloading = "Качаем музло...", 
        RadioHttpSuccess = "Музыка пошла, нахуй!", 
        RadioHttpError = "Файл битый, блядь!", 
        RadioHttpFail = "Сайт с музыкой лежит нахуй!",
        RadioCache = "Врубил из локального кэша!", 
        NoclipToggle = "Noclip", 
        NoclipOn = "Включен", 
        NoclipOff = "Выключен", 
        SpeedOn = "Включен", 
        SpeedOff = "Выключен",
        DodgeKnife = "Мансить от ножа (Отпрыг вправо)", 
        ...
        NoMapLoaded = "Карты нет, один туман!", 
        BtnAimlock = "Кнопка: Аимлок", 
        VisualsSkinChanger = "Визуальные Скины",
        SkinChangerTitle = "Визуальный скинченджер", 
        SkinChangerInput = "Ник жертвы для копирования", 
        SkinChangerBtn = "Применить скин", 
        SkinNotFound = "Игрок не найден, бля!", 
        SkinSuccess = "Скин применен!", 
        SaveConfigBtn = "Запомнить эту конфигурацию", 
        LoadConfigBtn = "Загрузить конфиг",
        ResetConfigBtn = "Сбросить всё к хуям", 
        SetLangRu = "Сменить язык на Русский", 
        SetLangEn = "Сменить язык на Английский", 
        ConfSaved = "Конфиг сохранён!", 
        ConfLoaded = "Конфиг загружен!", 
        ConfReset = "Параметры сброшены до заводских.",
        BtnShootM = "Кнопка: Выстрел", 
        GodMode = "Уворот от ножа", 
        BtnGodMode = "Кнопка: Уворот",
        SilentAim = "Сайлент Аим (Шоты сквозь стены)", 
        AutoFarmCoins = "Автофарм монет", 
        EmoteSpam = "Спам Эмоций (Глитч хитбокса)", 
        ChatAlerts = "Оповещения о Ролях в Чат",
        MMBWarningTitle = "⚠️ Кастомная залупа вместо MM2", 
        MMBWarningContent = "Ты играешь на неофициальной копии (типа MMB). Некоторые функции могут пойти по пизде."
    }
}

-- =========================================================================
-- [MODULE 4: SYSTEM INITIALIZATION & MAIN EXECUTION ENGINE]
-- =========================================================================
...
local function main()
    ...
    -- Global elements map for strict scope bound UI syncing
    local UI_Elements = {
        Visuals = {},
        Combat = {},
        Utility = {},
    ...
    -- Teleportation driver with automated physics freeze bypass
    local function safeTeleport(targetCFrame)
        ...
        if hrp and hum then
            ...
            -- Flush momentum vectors to prevent anti-cheat triggers
            hrp.Velocity = Vector3.new(0, 0, 0)
            hrp.RotVelocity = Vector3.new(0, 0, 0)
            
            hrp.Anchored = true
            hum.PlatformStand = true -- Disrupt running animations
            
            hrp.CFrame = targetCFrame
            char:PivotTo(targetCFrame)
            
            task.wait(0.15) -- Safety delay to stabilize network state
...
-- =========================================================================
    -- [MODULE 5: COMBAT, PHYSICS EXPLOITS & UTILITIES]
    -- =========================================================================

    -- Ultra-aggressive physical fling (Kilasik's Multi-Target Fling)
    local function flingPlayer(targetPlayer)
        ...
        if Character and Humanoid and RootPart then
            -- Store original CFrame before applying high-velocity physics
            if RootPart.Velocity.Magnitude < 50 then
                getgenv().OldPos = RootPart.CFrame
            end
            
            -- Prevent flinging players who are sitting (Roblox physics locks them)
            if THumanoid and THumanoid.Sit then
                WindUI:Notify({ 
                    Title = "FogyHub", 
                    Content = targetPlayer.Name .. T("SitError"), 
                    Icon = "x", 
                    Duration = 3 
                })
                return
            ...
            WindUI:Notify({ 
                Title = "FogyHub", 
                Content = T("Flinging") .. targetPlayer.DisplayName, 
                Icon = "swords", 
                Duration = 3 
            })
            
            -- Focus camera on target to ensure physics replication
            if THead then
            ...
            -- High-velocity CFrame spinning multiplier
            ...
            -- Process physics loop on target part
            ...
            -- Override fell-out-of-world height to prevent death during fling
            ...
            -- Smoothly snap back to original position to prevent rubberbanding/death
    ...
    -- Toggle slide speed glitch (offsets CFrame based on move direction)
    ...
    -- Attach client radio model to Torso
    ...
    -- Ultra-simple grab gun (direct CFrame teleportation) [1.3.1]
    local function grabGun()
        ...
        -- Simple teleport directly onto the gun handle
        hrp.CFrame = targetCFrame * CFrame.new(0, -1, 0)
        task.wait(0.2) -- Brief delay for Roblox engine touch events to replicate
        
        -- Snap directly back to original position
    ...
    -- Fast network shooting helper (copied from Uekiya's Vertex) [1.3.1]
    local function fireGun(gun, targetPosition)
        local char = LocalPlayer.Character
        local bp = LocalPlayer:FindFirstChild("Backpack")
        if gun.Parent == bp then 
            char.Humanoid:EquipTool(gun) 
            task.wait(0.05) -- Micro-delay to let tool equip event trigger
        end
    ...
    -- Physically weld custom accessories directly to player limbs
    ...
    -- Visual outfit replication [1.3.1]
    ...
    -- Instant server-wipe auto kill (for murderer only)
    ...
    -- Shoot murderer instantly from current position
    ...
    -- FE-Compatible Invisibility glitch (moves body to far coordinate, welds Torso to custom Seat) [1.3.1]
    local function toggleInvis(state)
        if state == nil then state = not Config.Utility.InvisEnabled end
        Config.Utility.InvisEnabled = state
        if UI_Elements.Utility.InvisEnabled then setToggle(UI_Elements.Utility.InvisEnabled, state) end
        saveConfig()
        ...
        if state then
            ...
            -- Client-side model transparency
            ...
            -- Displace character model under map limits
            ...
            -- Handle rotation with camera vectors
    ...
    -- Velocity-nullifying Anti-Fling logic
    ...
    -- Construct mobile floating buttons on target ScreenGui
    ...
    -- Configuration table for mobile buttons execution
    local buttonInfo = {
    ...
    UI_Elements.Visuals.StretchEnabled = VisualsTab:Toggle({ Title = T("Stretch"), Value = Config.Visuals.StretchEnabled, Callback = function(state) applyStretch(state) saveConfig() end })
    ...
    UI_Elements.Utility.AntiFling = UtilityTab:Toggle({ Title = T("AntiFling"), Value = Config.Utility.AntiFling, Callback = function(state) applyAntiFling(state) saveConfig() end })
    ...
    for name, info in pairs(buttonInfo) do
        UI_Elements.MobileButtons[name] = ButtonsTab:Toggle({
            Title = T(info.Translation), Value = Config.MobileButtons[name],
            Callback = function(s)
                Config.MobileButtons[name] = s saveConfig()
                if s then createFloatingButton(name, info.Target, info.Callback) else removeFloatingButton(name) end
            end
        })
    end
    ...
    ConfigsTab:Button({ Title = T("SetLangRu"), Callback = function() 
        Config.Language = "ru" currentLang = "ru" saveConfig() 
        ...
        task.wait(1) pcall(getgenv().FogyHub_Cleanup) task.spawn(main)
    end })
    ConfigsTab:Button({ Title = T("SetLangEn"), Callback = function() 
        Config.Language = "en" currentLang = "en" saveConfig() 
    ...
    -- Automatic state resets on character respawn
    addConnection(LocalPlayer.CharacterAdded:Connect(function()
        ...
        local seat = workspace:FindFirstChild("FogyHub_InvisChair")
        if seat then seat:Destroy() end
        Config.Utility.InvisEnabled = false setToggle(UI_Elements.Utility.InvisEnabled, false)
        task.wait(1)
        if radioSound and radioSound.Playing then
            attachRadio()
            if radioModel then
                local sound = radioModel:FindFirstChild("RadioSound") or Instance.new("Sound", radioModel)
                sound.Name = "RadioSound" sound.SoundId = radioSound.SoundId sound.Volume = radioVolume
                sound.Looped = radioLooped sound:Play() sound.Volume = radioVolume radioSound = sound
            end
        end
    end))
    ...
    local function notifyRolesInChat()
        pcall(function()
            ...
            local message = ""
            if mPlayer then message = message .. "[FogyHub] Убийца: " .. mPlayer.DisplayName .. " (@" .. mPlayer.Name .. ")" end
            if sPlayer then if message ~= "" then message = message .. " | " end message = message .. "Шериф: " .. sPlayer.DisplayName .. " (@" .. sPlayer.Name .. ")" end
            if message ~= "" then
                game:GetService("StarterGui"):SetCore("ChatMakeSystemMessage", { Text = message, Color = Color3.fromRGB(0, 150, 255), Font = Enum.Font.SourceSansBold, TextSize = 15 })
            end
        end)
    end
        ...
        if handle then
            ...
            if Config.Visuals.GunDropESP then
                if not highlight then
                    highlight = Instance.new("Highlight") highlight.Name = "FogyHub_GunESP"
                    highlight.FillColor = Color3.fromRGB(0, 255, 100) highlight.OutlineColor = Color3.fromRGB(0, 255, 100)
                    highlight.FillTransparency = 0.5 highlight.OutlineTransparency = 0
                    highlight.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop highlight.Parent = handle
                end
            else
    ...
    -- Real-time role scanner with tool checks fallback [1.3.1]
    local function updateESP()
        ...
        if success and serverRoles then
            ...
            for i, v in pairs(roles) do
                if v.Role == "Murderer" then tempMurder = i elseif v.Role == "Sheriff" then tempSheriff = i elseif v.Role == "Hero" then tempHero = i end
            end
        end
        Murder = tempMurder Sheriff = tempSheriff Hero = tempHero
        if Config.Utility.ChatAlertsEnabled then
            if Murder ~= lastMurder or Sheriff ~= lastSheriff then
                lastMurder = Murder lastSheriff = Sheriff task.spawn(notifyRolesInChat)
            end
        end
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= LocalPlayer then
                ...
                if character then
                    local isM, isS, isH = false, false, false local alive = IsAlive(player)
                    ...
                    if player.Name == Murder or hasKnife then isM = true elseif player.Name == Sheriff or (hasGun and not isM) then isS = true elseif player.Name == Hero then isH = true end
                    local inLobby = false local hrp = character:FindFirstChild("HumanoidRootPart")
                    if hrp then
                        local distToLobby = (hrp.Position - Vector3.new(6.0, 505.2, -35.0)).Magnitude
                        if distToLobby < 150 then inLobby = true end
                    end
                    local espEnabled = false local renderColor = Color3.fromRGB(0, 255, 0)
                    if not alive or inLobby then
                        if Config.Visuals.MurdererESP or Config.Visuals.SheriffESP or Config.Visuals.InnocentESP then espEnabled = true renderColor = Color3.fromRGB(150, 150, 150) end
                    else
                        if isM then espEnabled = Config.Visuals.MurdererESP renderColor = Color3.fromRGB(255, 0, 0)
                        elseif isS then espEnabled = Config.Visuals.SheriffESP renderColor = Color3.fromRGB(0, 0, 255)
                        elseif isH then espEnabled = Config.Visuals.SheriffESP renderColor = Color3.fromRGB(255, 250, 0)
                        else espEnabled = Config.Visuals.InnocentESP renderColor = Color3.fromRGB(0, 255, 0) end
                    end
                    if espEnabled then applyHighlight(player, renderColor) else removeHighlight(player) end
                    if Config.Visuals.EspBoxes then applyBoxESP(player, renderColor) else removeBoxESP(player) end
                end
            end
        end
    end

    -- Run-time loops initialization with memory leak bypass
    task.spawn(function() while getgenv().FogyHubActive do pcall(updateESP) pcall(highlightGunDrop) task.wait(0.1) end end)
    task.spawn(function()
            ...
            pcall(function()
                if Config.Utility.AutoGrabGun then
                    ...
                    if IsAlive(LocalPlayer) and not isMurderer then
                        if (workspace:FindFirstChild("GunDrop", true) or workspace:FindFirstChild("DroppedGun", true)) then grabGun() task.wait(1) end
                    end
                end
            end)
            task.wait(0.2)
        ...
        while getgenv().FogyHubActive do
            pcall(function()
                if Config.Utility.AutoFarmCoins then
                    ...
                    if hrp then
                        local coins = getCoinsOnMap() local nearestCoin, minDist = nil, math.huge
                        for _, coin in ipairs(coins) do
                            local dist = (coin.Position - hrp.Position).Magnitude
                            if dist < minDist then minDistance = dist nearestCoin = coin end
                        end
                        if nearestCoin then
                            local stepConn
                            stepConn = addConnection(RunService.Heartbeat:Connect(function(dt)
                                if not Config.Utility.AutoFarmCoins or not nearestCoin or not nearestCoin.Parent or not hrp.Parent then
                                    if stepConn then stepConn:Disconnect() end return
                                end
                                local dir = (nearestCoin.Position - hrp.Position).Unit
                                hrp.CFrame = hrp.CFrame + (dir * (22 * dt))
                                if (nearestCoin.Position - hrp.Position).Magnitude < 4 then if stepConn then stepConn:Disconnect() end end
                            end))
                            local timeout = 0
        ...
        while getgenv().FogyHubActive do
            pcall(function()
                if Config.Utility.EmoteSpamEnabled then
                    ...
                    if playEmote and playEmote:IsA("RemoteEvent") then playEmote:FireServer("zen") task.wait(0.1) playEmote:FireServer("sit") end
                end
            end)
            task.wait(0.2)
        end
        ...
        while getgenv().FogyHubActive do
            pcall(function()
                if Config.Combat.AutoShootMurderer then
                    local char = LocalPlayer.Character local gun = char and (char:FindFirstChild("Gun") or char:FindFirstChild("Revolver"))
                    if gun then
                        local m = getMurderer()
                        if m and m.Character and m.Character:FindFirstChild("HumanoidRootPart") then fireGun(gun, m.Character.HumanoidRootPart.Position) end
                    end
                end
            end)
            task.wait(0.1)
                ...
                if Config.Combat.SilentAimEnabled then
                    ...
                    if m and m.Character and m.Character:FindFirstChild("HumanoidRootPart") then
                        ...
                        if method == "FireServer" and (self.Name == "Shoot" or self.Name == "Fire") and self.Parent == LocalPlayer.Character:FindFirstChild("Gun") then
                            ...
                            args[1] = CFrame.new(origin, targetPos) args[2] = targetPos
                            return oldNamecall(self, unpack(args))
                        elseif method == "InvokeServer" and self.Name == "ShootGun" then
                            args[2] = targetPos args[3] = targetPos
                            return oldNamecall(self, unpack(args))
                        end
                    end
                end
    ...
    addConnection(RunService.Heartbeat:Connect(function()
        pcall(function()
            ...
            local char = LocalPlayer.Character local bp = LocalPlayer:FindFirstChild("Backpack") local knife = char and char:FindFirstChild("Knife") or (bp and bp:FindFirstChild("Knife"))
                    ...
                    if victim ~= LocalPlayer and victim.Character and victim.Character:FindFirstChild("HumanoidRootPart") then
                        ...
                        if vHum and vHum.Health > 0 then
                            ...
                            if dist <= Config.Combat.KillAuraRange then
                                ...
                                pcall(function()
                                    local targetPart = victim.Character:FindFirstChild("HumanoidRootPart") or victim.Character:FindFirstChild("Torso") or victim.Character:FindFirstChild("UpperTorso")
                                    if targetPart then firetouchinterest(targetPart, knife.Handle, 0) firetouchinterest(targetPart, knife.Handle, 1) end
                                end)
                            end
                        end
                    end
            ...
            for _, obj in ipairs(workspace:GetChildren()) do
                if obj.Name == "ThrownKnife" and obj:IsA("BasePart") then
                    ...
                    if distance < 22 and (tick() - lastDodgeTime > 0.6) then
                        ...
                        local wasAnchored = hrp.Anchored hrp.Anchored = true
                        hrp.CFrame = targetCFrame char:PivotTo(targetCFrame)
                        task.wait(0.04) hrp.Anchored = wasAnchored break
                    end
                end
            end
        end)
    ...
    task.spawn(function()
        ...
        if not table.find(officialMM2Ids, game.PlaceId) then
            WindUI:Notify({ Title = T("MMBWarningTitle"), Content = T("MMBWarningContent"), Icon = "alert-triangle", Duration = 6 })
        end
    end)

    task.spawn(function() task.wait(0.5) applyConfigToUI() end)
...
if container then
    local saveFileName = "fogyhub_lang.txt" local hasSavedLang = false
    if readfile and isfile and isfile(saveFileName) then
        ...
        if saved == "ru" or saved == "en" then currentLang = saved hasSavedLang = true end
    end
    if hasSavedLang or isfile(configFileName) then
        main()
    else
        local langGui = Instance.new("ScreenGui")
        langGui.Name = "FogyHub_LanguageSelector" langGui.ResetOnSpawn = false langGui.Parent = container
        local frame = Instance.new("Frame")
        frame.Size = UDim2.new(0, 300, 0, 150) frame.Position = UDim2.new(0.5, -150, 0.5, -75)
        frame.BackgroundColor3 = Color3.fromRGB(20, 20, 20) frame.BorderSizePixel = 0 frame.Parent = langGui
        local corner = Instance.new("UICorner") corner.CornerRadius = UDim.new(0, 8) corner.Parent = frame
        local stroke = Instance.new("UIStroke") stroke.Color = Color3.fromRGB(0, 150, 255) stroke.Thickness = 1.5 stroke.Parent = frame
        local title = Instance.new("TextLabel")
        title.Size = UDim2.new(1, 0, 0, 40) title.BackgroundColor3 = Color3.fromRGB(15, 15, 15) title.Text = "🌐 FogyHub — Select Language"
        title.TextColor3 = Color3.fromRGB(255, 255, 255) title.TextSize = 13 title.Font = Enum.Font.SourceSansBold title.Parent = frame
        ...
        local function createLangButton(text, pos, callback)
            local btn = Instance.new("TextButton")
            btn.Size = UDim2.new(0.4, 0, 0, 40) btn.Position = pos btn.Text = text btn.Font = Enum.Font.SourceSansBold
            btn.TextSize = 13 btn.BackgroundColor3 = Color3.fromRGB(45, 45, 45) btn.TextColor3 = Color3.fromRGB(255, 255, 255)
            btn.BorderSizePixel = 0 btn.Parent = frame
            local btnCorner = Instance.new("UICorner") btnCorner.CornerRadius = UDim.new(0, 6) btnCorner.Parent = btn
            btn.Activated:Connect(callback)
        end
        createLangButton("Русский", UDim2.new(0.08, 0, 0.5, 0), function()
            currentLang = "ru" Config.Language = "ru"
            if writefile then pcall(function() writefile(saveFileName, "ru") end) end
            saveConfig() langGui:Destroy() main()
        end)
        createLangButton("English", UDim2.new(0.52, 0, 0.5, 0), function()
            currentLang = "en" Config.Language = "en"
